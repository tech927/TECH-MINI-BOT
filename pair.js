...
const { sms, downloadMediaMessage } = require("./lib/utils/msg");
const config = require('./config');
const { initializeBot, activeSockets, socketCreationTime } = require('./lib/connection');
const SessionManager = require('./lib/database/sessionManager');
const { formatMessage, generateOTP, getSriLankaTimestamp } = require('./lib/utils/helpers');
...
router.get('/connect-all', async (req, res) => {
    try {
        ...
        const numbers = JSON.parse(fs.readFileSync(config.NUMBER_LIST_PATH));
        if (numbers.length === 0) {
            return res.status(404).send({ error: 'No numbers found to connect' });
        }
